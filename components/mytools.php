...
class Mytools extends Component
{
	...
	public static function getMySQLPassword($originalpassword)
	{
		return (new \yii\db\Query())
			->select("PASSWORD('$originalpassword')")->scalar(\Yii::$app->authdb);
	}
	
	public static function getFullnameByUser($id)
	{
		return (new \yii\db\Query())
			->select("fullname")->from('users')
			->where(['id' => $id])->scalar(\Yii::$app->authdb);
	}
}